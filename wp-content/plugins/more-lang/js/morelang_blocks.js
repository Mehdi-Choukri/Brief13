...
function mlLocalizeGutenbergEditor( $toolbar ) {
	...
	var DFT_autosaveInterval = core_editor.getEditorSettings().autosaveInterval;
	if ( isNaN( parseInt(DFT_autosaveInterval) ) ) DFT_autosaveInterval = 99999;
	var localeTitles = {};
	var localeContents = {};
	var localeExcerpts = {};
	var localeMetas = {};
	...
	/* Autosave is not supported in the Gutenberg editor */
	function disableAutosave() {
		core_editor_d.updateEditorSettings({autosaveInterval: 99999});
	}
	function restoreAutosave() {
		core_editor_d.updateEditorSettings({autosaveInterval: DFT_autosaveInterval});
	}
	...
	function createSwitchFunc(idxToShow) {
		return function() {
			...
			curIdx = idxToShow;
			curIdx>0 ? disableAutosave() : restoreAutosave();
	...
	function submitTranslation() {
		...
		wp.apiFetch({
			...
			if ( typeof resp === "object" && resp && resp.message ) {
				if ( resp.code === 0 ) {
					core_notices_d.createSuccessNotice("More-Lang: " + resp.message);
				}
				else {
