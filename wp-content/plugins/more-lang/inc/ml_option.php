...
/**
 * Enqueue the Select2, a third-party JS library.
 */
function ml_enqueue_select2() {
	$cur_plugin_url = plugin_dir_url( dirname(__FILE__) ); // Will link to the More-Lang, even if called from another plugin
		...
		<table id="ml-langcfg-tbl">
			<thead>
				<tr>
					...
					<th><?php esc_html_e( 'Flag', ML_TDOMAIN ) ?>
					    <?php ml_tooltip( sprintf( __('Saved in <em>~/plugins/%s/cflag</em>, you can copy your own flags to it', ML_TDOMAIN), ML_DIR_NAME ) ) ?>
			...
			<tfoot>
				<tr><td colspan="8">
					...
					<label for="ml-auto-chooser"><?php esc_html_e( 'Auto-generate the language switcher', ML_TDOMAIN ) ?></label>
					<span class="ml-auto-chooser-hight-fix"></span>
					<span id="ml-pos-input-grp"><label><?php esc_html_e( ' at:', ML_TDOMAIN ) ?></label>
					...
					<label for="ml-pos-inputmode-sel"><?php esc_html_e( 'Fixed Position', ML_TDOMAIN ) ?></label>
					...
					<?php ml_tooltip( __('Append the switcher(s) to the element(s) specified by the CSS Selector.', ML_TDOMAIN) ) ?>
